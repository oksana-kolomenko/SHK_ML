import numpy as np
from helpers import load_labels, load_features, load_summaries, logistic_regression, lr_ran_tree_emb, lr_txt_emb, \
     hgbc, hgbc_txt_emb
...
def run_all_models():
    ...
    # 3. text embedding based on [CLS] token
    #text_emb_cls_train_score, text_emb_cls_test_scores = \
     #   lr_txt_emb(embeddings=patient_summaries, y=y)
    ...
    # 6. text embedding based on [CLS] token + hgbc
    #text_emb_hgbc_cls_train_score, text_emb_hgbc_cls_test_scores = \
     #   hgbc_txt_emb(embeddings=patient_summaries, nominal_features=nominal_features, y=y)
    ...
    # plot the bars
    plot_bar_chart(filename="bar_chart_hgbc", labels=labels, train_scores=train_scores,
